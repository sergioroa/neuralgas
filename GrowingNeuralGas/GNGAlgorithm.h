...
*/
template<typename T,typename S> GNGAlgorithm<T,S>::GNGAlgorithm<T,S>(const int& dim): GNGModul<T,S>(dim)
{_graphptr=NULL;}
...
*/
template<typename T,typename S> T GNGAlgorithm<T,S>::getDistance(const int& time, const int& node_index)
{
